  ...
  return (
    <div className={styles.home}>
        ...
        <div className={styles.carouselContainer}>
            {artistList.status!=null && artistList.status==200?<SlickCarousel data={artistList.data.slice(sliceStart,sliceStart+10)} />:<Error404 />}
            
        </div>
        </section>
        <section className={styles.topPicks}>
        <h1 className={styles.sectionHeading}>Top Picks</h1>
        {topPicks.status!=null && topPicks.status==200?<ImageSlider data={topPicks.data} /> :<Error404 />}
        </section>
        <section className={styles.recent}>
        <h1 className={styles.sectionHeading}>Recent Uploads</h1>
        {recent.status!=null && recent.status==200?<ImageSlider data={recent.data} />  :<Error404 />}
        </section>
    </div>
  )
}
